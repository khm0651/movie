...
let vidioWrapper = document.querySelector(".vidio-wrapper");
let iframe = document.querySelector("iframe");
let vidioClose = document.querySelector(".vidio-close");
...
makeFilter = (parent) =>{
	...
	input.type="button";
	input.value="네이버 리뷰 보기";
	input.classList.add("naver-review-btn");
	div.append(input);
	
	input = document.createElement("input");
	input.type="button";
	input.value="유튜브 리뷰 보기";
	input.classList.add("youtube-review-btn");
	div.append(input);
...
for(let i = 0; i<img.length; i++){
	img[i].addEventListener("mouseover",e=>{
		if(!showFilter){
			showFilter= true;
			let filter = makeFilter(e.target.parentElement);
			...
			filter.children[1].addEventListener("click",e=>{
				...
				if(movieName[i].className != "notSearch"){
					vidioWrapper.classList.add("vidio-show");
					iframe.src = movieName[i].className;
					vidioClose.addEventListener("click",e=>{
						vidioWrapper.classList.remove("vidio-show");
						iframe.src="";
					})
				}else{
